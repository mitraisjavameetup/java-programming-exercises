...
public class StreamExamples {
	...
	public String uppercaseWithoutMap(){
		 ...
		 */
		uppercaseString = words.stream().reduce("", String::concat);
	...
	public int numberOfCharacters(){
		 ...
		 *using lambda expression
		 */
		numOfChar = words.stream().reduce("",String::concat).length();
		return numOfChar;
	}
	...
	public int numberOfWordsWithH(){
		 ...
		 *using lambda expression
		 */
		numOfH = (int) words.stream().filter(e->e.contains("h")).count();
		return numOfH;
	}
}
