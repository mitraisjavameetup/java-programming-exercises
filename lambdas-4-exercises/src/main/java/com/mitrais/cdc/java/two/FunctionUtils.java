...
public class FunctionUtils {
	...
	// @SafeVarargs is difficult to understand. The issue is that it is not
	// always safe to use varargs for generic types:
	// the resultant array can have runtime type problems if you modify entries
	// in it.
	// @SafeVarargs says "I am not doing anything dangerous, please suppress the
	// compiler warnings".
	// For details, see
	...
	public static <T> Predicate<T> allPassPredicate(Predicate<T>... tests) {
		...
		/**
		 * TODO: create a 'for' loop. Returns a Predicate that is the result of
		 * ANDing all the argument Predicates. If no Predicates are supplied, it
	...
	public static <T> T firstAllMatch(Stream<T> elements, Predicate<T>... tests) {
		...
		// TODO: filter based first element that matches ALL of the tests, null
		// otherwise
