...
public class MathUtils {
	...
	public static void markPrimesSerial(boolean[] primeFlags) {
		...
		TimingUtils.timeOp(() -> {
			markPrimesSerial(primeFlags, 0, primeFlags.length);
			return "markPrimesSerial with " + primeFlags.length + " node";
		});
	...
	public static void markPrimesParallel(boolean[] primeFlags) {
		...
		TimingUtils.timeOp(() -> {
			FORK_JOIN_POOL.invoke(new ParallelPrimeMarker(primeFlags, 0, primeFlags.length));
			return "markPrimesParallel with " + primeFlags.length + " node";
		});
	...
	public static List<Integer> findPrimesSerial(int size) {
		...
		for (int i = 0; i < size; i++) {
			if(Primes.isPrime(i)) list.add(i);
		}
		return list;
	}
	...
	public static List<Integer> findPrimesParallel(int size) {
		// To Do : collect and return list of marked prime numbers from given
		// only the size
		return findPrimesParallel(new boolean[size]);
	}
