...
public class StreamExamples {
    ...
    public static List<String> excitingWordsStreams() {
	   ...
	   */
        ...
        return words.stream().map(m -> m + "!").collect(Collectors.toList());
    ...
    public static List<String> eyeWordsStreams() {
	   ...
	   * 
	   */
        return words.stream().map(m -> m.replace("i", "eye")).collect(Collectors.toList());
    }
    ...
    public static List<String> evenLengthWords() {
	  ...
	  /*
	   * TO DO
	   * THIS INSTRUCTION IS MISLEADING, SHOULD BE : FIND ALL WORDS WITH EVEN CHARACTERS IN IT
	   * -- WRONG -- Produce the same lists as
	   * -- WRONG -- List<String> wordsWithB = StringUtils.allMatches(words, s -> s.contains("b"));
	   * -- WRONG -- but this time use “filter”
	   * 
	   */
        List<String> b = words.stream().filter(s -> s.length() % 2 == 0).collect(Collectors.toList());
        return b;
    }
    ...
    public static String firstFunnyString(String containedTest) {
	   ...
	   */
        return words.stream().map(m -> m.toUpperCase()).filter(s -> s.length() < 4 && s.contains(containedTest)).findAny().orElse("No match");
    }
    ...
    public static String[] wordsAsArray() {
	  ...
	  * 
	  */
        return words.stream().toArray(size -> new String[size]);
    }
