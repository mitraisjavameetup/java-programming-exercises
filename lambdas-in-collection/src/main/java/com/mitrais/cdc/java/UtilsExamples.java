...
public class UtilsExamples { 
  ...
  public static double showAverages1(int size, double cutoff) {
	   ...
	   *
	   */
	  List<Double> initList = Utils.randomNumberList(size);
	  double average = Utils.average(initList);
	  Utils.removeBelow(initList, cutoff);
	  average = Utils.average(initList);
  ...
  public static double showAverages2(int size) {
	  ...
	  List<Double> initList = Utils.randomNumberList(size);
	  double average = Utils.average(initList);
	  Utils.doubleAllValues(initList);
	  average = Utils.average(initList);
	  return average;
